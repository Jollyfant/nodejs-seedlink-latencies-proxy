...
SeedlinkInfoSocket.prototype.parseRecords = function(records) {
  ...
  libxmljs.parseXml(records.join("")).root().childNodes().forEach(function(station) {

    // Go over all children (streams)
    station.childNodes().forEach(function(stream) {

      // Skip identifiers that do not have quality D
      if(stream.getAttribute("type").value() !== "D") {
        return;
      }

      // Get the end time from Seedlink
      var end = Date.parse(stream.getAttribute("end_time").value() + " GMT");

      // Collect all latencies
      latencies.push({
        "network": station.getAttribute("network").value(),
        "station": station.getAttribute("name").value(),
        "location": stream.getAttribute("location").value(),
        "channel": stream.getAttribute("seedname").value(),
